# vinifica Strategy Core

The vinifica-strategy-core package is a foundational library designed to facilitate the development of strategies for vinifica. It provides reusable abstractions and utilities that streamline the creation, management, and integration of strategies into the vinifica ecosystem.
...
3. **Core Traits**: Defines the vinificaStrategyTrait, which serves as the contract for developing new strategies.

### Installation

Add the vinifica-strategy-core package to your Cargo.toml dependencies:

```toml
[dependencies]
vinifica-strategy-core = "0.2.0"
```
...
```rust
use vinifica_strategy_core::{vinificaStrategyTrait, StrategyError, event};
```

2. Implement the Strategy Trait

Define your custom strategy by implementing the vinificaStrategyTrait:
```rust
#[contract]
struct MyCustomStrategy;

#[contractimpl]
impl vinificaStrategyTrait for MyCustomStrategy {
