...
import { depositToVault, getCreatevinificaParams, rebalanceManager, withdrawFromVault } from "./utils/vault.js";
...
export async function test_factory(addressBook: AddressBook) {
  ...
  console.log("-------------------------------------------------------");
  console.log("Testing Create vinifica on Factory");
  console.log("-------------------------------------------------------");

  console.log("Setting Emergengy Manager, Fee Receiver and Manager accounts");
  const emergencyManager = loadedConfig.getUser("vinifica_EMERGENCY_MANAGER_SECRET_KEY");
  if (network !== "mainnet") await airdropAccount(emergencyManager);

  const feeReceiver = loadedConfig.getUser("vinifica_FEE_RECEIVER_SECRET_KEY");
  if (network !== "mainnet") await airdropAccount(feeReceiver);

  const manager = loadedConfig.getUser("vinifica_MANAGER_SECRET_KEY");
  ...
  const createvinificaParams: xdr.ScVal[] = getCreatevinificaParams(
    emergencyManager,
    rebalanceManager,
    feeReceiver,
    manager,
  ...
  const result = await invokeContract(
    ...
    createvinificaParams,
    loadedConfig.admin
  );

  console.log('🚀 « vinifica Vault created with address:', scValToNative(result.returnValue));
