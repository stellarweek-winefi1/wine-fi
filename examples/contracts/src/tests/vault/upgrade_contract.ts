...
import { deployvinificaVault } from "./utils.js";
...
export async function testUpgradeContract(addressBook: AddressBook, params: CreateVaultParams[]) {
  try {
    ...
    console.log(yellow, "----------------");
    await installContract("vinifica_vault", addressBook, admin);
    console.log(green, "----------------");
  ...
  } = await deployvinificaVault(addressBook, params);
  ...
  await (async () => {
    try {
      ...
      const wasm_hash = new Uint8Array(Buffer.from(addressBook.getWasmHash("vinifica_vault"), "hex"));
