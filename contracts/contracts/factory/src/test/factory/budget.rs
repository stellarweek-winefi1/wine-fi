...
use crate::test::{create_asset_params, create_vinifica_factory, vinificaFactoryTest};
...
fn budget() {
    ...
    let factory_contract = create_vinifica_factory(
        &test.env,
        &test.admin,
        &test.vinifica_receiver,
        2000u32,
        &test.vinifica_wasm_hash,
    );

    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();

    std::println!("create_vinifica_factory()                                              | cpu: {},      mem: {}", cpu, mem);

    test.env.cost_estimate().budget().reset_unlimited();

    // create vinifica vault
    ...
    factory_contract.create_vinifica_vault(
        &roles,
        &2000u32,
        &asset_params,
    ...
    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
    std::println!("create_vinifica_vault()                                                | cpu: {},      mem: {}", cpu, mem);

    test.env.cost_estimate().budget().reset_unlimited();
    // create vinifica vault deposit
    let asset_params = create_asset_params(&test);
    ...
    test.factory_contract.create_vinifica_vault_deposit(
        &test.manager,
        &roles,
        &2000u32,
        &asset_params,
    ...
    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
    std::println!("create_vinifica_vault_deposit()                                        | cpu: {},      mem: {}", cpu, mem);
}
