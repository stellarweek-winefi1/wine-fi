...
fn set_new_admin_by_admin() {
    ...
    let users = vinificaFactoryTest::generate_random_users(&test.env, 1);
...
#[test]
fn set_vinifica_receiver_by_admin() {
    let test = vinificaFactoryTest::setup();
    ...
    test.factory_contract
        .mock_auths(&[MockAuth {
            ...
            invoke: &MockAuthInvoke {
                contract: &test.factory_contract.address.clone(),
                fn_name: "set_vinifica_receiver",
                args: (&users[0],).into_val(&test.env),
                sub_invokes: &[],
            },
        }])
        .set_vinifica_receiver(&users[0]);

    let expected_auth = AuthorizedInvocation {
        function: AuthorizedFunction::Contract((
            test.factory_contract.address.clone(),
            Symbol::new(&test.env, "set_vinifica_receiver"),
            (users[0].clone(),).into_val(&test.env),
        )),
        sub_invocations: vec![],
    };
    ...
    let new_fee_receiver: Address = test.factory_contract.vinifica_receiver();
...
fn set_vault_wasm_hash_by_admin() {
    ...
    let wasm_hash_events: std::vec::Vec<(Address, Vec<Val>, Val)> = events
        ...
        .filter(|event| {
            event.1 == sorobanvec![
                &test.env,
                String::from_str(&test.env, "vinificaFactory").into_val(&test.env),
