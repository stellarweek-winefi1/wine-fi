...
pub trait FactoryTrait {
    ...
    /// * `vinifica_receiver` - The default address designated to receive a portion of fees.
    /// * `vinifica_fee` - The initial annual fee rate (in basis points).
    /// * `vault_wasm_hash` - The hash of the vinifica Vault's WASM file for deploying new vaults.
    ///
    /// # Returns
    ...
    /// Creates a new vinifica Vault with specified parameters.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ...
    /// Creates a new vinifica Vault with specified parameters and makes the first deposit to set ratios.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ...
    fn create_vinifica_vault_deposit(
        e: Env,
        caller: Address,
        roles: Map<u32, Address>,
        vault_fee: u32,
    ...
    /// Updates the default receiver address for the vinifica portion of fees.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ...
    /// # Returns
    /// * `Result<(), FactoryError>` - Returns Ok(()) if successful, or an error if not authorized.
    fn set_vinifica_receiver(e: Env, new_fee_receiver: Address) -> Result<(), FactoryError>;

    /// Updates the default fee rate for new vaults.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    /// * `vinifica_fee` - The new annual fee rate in basis points.
    ///
    /// # Returns
    /// * `Result<(), FactoryError>` - Returns Ok(()) if successful, or an error if not authorized.
    fn set_vinifica_fee(e: Env, vinifica_fee: u32) -> Result<(), FactoryError>;
    ...
    /// Retrieves the current vinifica receiver's address.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ///
    /// # Returns
    /// * `Result<Address, FactoryError>` - Returns the vinifica receiver's address or an error if not found.
    fn vinifica_receiver(e: Env) -> Result<Address, FactoryError>;

    /// Retrieves the total number of deployed vinifica vaults.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ///
    ...
    fn vinifica_fee(e: Env) -> Result<u32, FactoryError>;
...
fn create_vault_internal(
    ...
    let vinifica_receiver = get_vinifica_receiver(e)?;
    let vinifica_fee = get_fee_rate(e)?;
    ...
    init_args.push_back(vinifica_receiver.to_val());
    init_args.push_back(vinifica_fee.into_val(e));
    ...
    let vinifica_address = create_contract(e, vault_wasm_hash, init_args);
    add_new_vault(e, vinifica_address.clone());

    events::emit_create_vinifica_vault(
        &e,
        roles,
        vault_fee,
        assets,
...
#[contractimpl]
impl FactoryTrait for vinificaFactory {
    /// Initializes the factory contract with the given parameters.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    /// * `admin` - The address of the contract administrator who can manage settings.
    /// * `vinifica_receiver` - The default address designated to receive the vinifica portion of fees.
    /// * `vinifica_fee` - The initial fee rate in basis points (1 basis point = 0.01%).
    /// * `vault_wasm_hash` - The hash of the vinifica Vault's WASM file used for deploying new vaults.
    ///
    /// # Behavior
    /// 1. Sets the initial admin address
    /// 2. Sets the initial vinifica fee receiver address
    /// 3. Sets the initial vault WASM hash
    /// 4. Sets the initial vinifica fee rate
    ...
    fn __constructor(
        ...
        put_admin(&e, &admin);
        put_vinifica_receiver(&e, &vinifica_receiver);
        put_vault_wasm_hash(&e, vault_wasm_hash);
        extend_instance_ttl(&e);
        put_vinifica_fee(&e, &vinifica_fee)?;
        Ok(())
    }

    /// Creates a new vinifica Vault with the specified parameters.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    /// * `roles` - A `Map` containing role identifiers (`u32`) and their corresponding `Address` assignments.
    ...
    /// Creates a new vinifica Vault with specified parameters and performs an initial deposit to set asset ratios.
    ///
    /// # Arguments
    /// * `e` - The environment in which the contract is running.
    ...
    fn set_vinifica_receiver(e: Env, new_fee_receiver: Address) -> Result<(), FactoryError> {
        ...
        put_vinifica_receiver(&e, &new_fee_receiver);
        events::emit_new_vinifica_receiver(&e, new_fee_receiver);
        Ok(())
    }
    ...
    fn set_vinifica_fee(e: Env, vinifica_fee: u32) -> Result<(), FactoryError> {
        ...
        put_vinifica_fee(&e, &vinifica_fee)?;
        events::emit_new_vinifica_fee(&e, vinifica_fee);
        Ok(())
    }
