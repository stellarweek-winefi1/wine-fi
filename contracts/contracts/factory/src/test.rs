#![cfg(test)]
extern crate std;
use crate::{vinificaFactory, vinificaFactoryClient};
...
fn create_vinifica_factory<'a>(
    e: &Env,
    admin: &Address,
    vinifica_receiver: &Address,
    vinifica_fee: u32,
    vinifica_wasm_hash: &BytesN<32>,
) -> vinificaFactoryClient<'a> {
    let args = (admin, vinifica_receiver, vinifica_fee, vinifica_wasm_hash);
    vinificaFactoryClient::new(e, &e.register(vinificaFactory, args))
}

// vinifica Vault Contract
mod vinifica_vault_contract {
    soroban_sdk::contractimport!(
...
pub(crate) fn create_asset_params(test: &vinificaFactoryTest) -> Vec<AssetStrategySet> {
    sorobanvec![
        &test.env,
        AssetStrategySet {
...
pub struct vinificaFactoryTest<'a> {
    env: Env,
    factory_contract: vinificaFactoryClient<'a>,
    vinifica_wasm_hash: BytesN<32>,
    admin: Address,
    vinifica_receiver: Address,
...
impl<'a> vinificaFactoryTest<'a> {
    fn setup() -> Self {
        ...
        let factory_contract = create_vinifica_factory(
            &env,
            &admin,
            &vinifica_receiver,
            100u32,
