...
// Fee Receiver
pub fn put_vinifica_receiver(e: &Env, address: &Address) {
    e.storage()
        .instance()
        .set(&DataKey::vinificaReceiver, address);
}

pub fn get_vinifica_receiver(e: &Env) -> Result<Address, FactoryError> {
    e.storage()
        .instance()
        .get(&DataKey::vinificaReceiver)
