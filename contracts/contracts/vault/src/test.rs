...
// vinifica Hodl Strategy Contract
...
pub mod vinifica_vault {
    soroban_sdk::contractimport!(
        file = "../target/wasm32v1-none/release/vinifica_vault.optimized.wasm"
    );
    pub type vinificaVaultClient<'a> = Client<'a>;
}
use vinifica_vault::{AssetStrategySet, vinificaVaultClient, Strategy};

pub fn create_vinifica_vault<'a>(
    e: &Env,
    ...
    upgradable: bool,
) -> vinificaVaultClient<'a> {
    let args = (
        assets,
        roles,
        vault_fee,
        vinifica_protocol_receiver,
        vinifica_protocol_rate,
        soroswap_router,
        name_symbol,
        upgradable
    );
    let address = &e.register(vinifica_vault::WASM, args);
    let client = vinificaVaultClient::new(e, address);
    client
}
...
pub(crate) fn create_strategy_params_token_0(test: &vinificaVaultTest) -> Vec<Strategy> {
    create_strategy_params(test, test.strategy_client_token_0.address.clone())
}
pub(crate) fn create_unsafe_strategy_params_token_0(test: &vinificaVaultTest) -> Vec<Strategy> {
    sorobanvec![
        &test.env,
        Strategy {
...
pub(crate) fn create_strategy_params_token_1(test: &vinificaVaultTest) -> Vec<Strategy> {
    create_strategy_params(test, test.strategy_client_token_1.address.clone())
}

pub(crate) fn create_strategy_params(test: &vinificaVaultTest, address: Address) -> Vec<Strategy> {
    sorobanvec![
        &test.env,
        Strategy {
            name: String::from_str(&test.env, "Strategy 1"),
...
pub(crate) fn create_fixed_strategy_params_token_0(test: &vinificaVaultTest) -> Vec<Strategy> {
    sorobanvec![
        &test.env,
        Strategy {
...
pub struct vinificaVaultTest<'a> {
    env: Env,
...
impl<'a> vinificaVaultTest<'a> {
    fn setup() -> Self {
        ...
        let vinifica_protocol_receiver = Address::generate(&env);
