...
use crate::test::{
    create_vinifica_vault, create_strategy_params_token_0, create_strategy_params_token_1, create_strategy_params,
    vinificaVaultTest,
};
use crate::deposit;
...
fn amounts_desired_less_length() {
    ...
    let response = vinifica_contract.try_deposit(
...
fn one_asset_success() {
    ...
    let total_managed_funds = vinifica_contract.fetch_total_managed_funds().get(0).unwrap();
...
fn one_asset_min_more_than_desired() {
    ...
    // deposit
    let result = vinifica_contract.try_deposit(
        &sorobanvec![&test.env, amount],
...
fn several_assets_success() {
    ...
    // check that current invested funds is now 0, funds still in idle funds
    let current_invested_funds_0 = vinifica_contract.fetch_total_managed_funds().get(0).unwrap().invested_amount;
    let current_invested_funds_1 = vinifica_contract.fetch_total_managed_funds().get(1).unwrap().invested_amount;
...
fn several_assets_min_greater_than_optimal() {
    ...
    // deposit
    let deposit_result = vinifica_contract.try_deposit(
        &sorobanvec![&test.env, amount0, amount1],
...
fn amounts_desired_zero() {
    ...
    let vault_balance_before = test.token_0.balance(&vinifica_contract.address);
    assert_eq!(vault_balance_before, 0i128);

    let df_balance_before = vinifica_contract.balance(&users[0]);
...
fn with_zero_amounts() {
    ...
    let total_supply = vinifica_contract.total_supply();
    let user_balance = vinifica_contract.balance(&users[0]);
    ...
    let user_balance = vinifica_contract.balance(&users[1]);
    let total_supply = vinifica_contract.total_supply();
...
fn mint_zero_shares(){
    ...
    // mint zero shares
    let _ = test.env.as_contract(&vinifica_contract.address, || deposit::test_mint_shares(&test.env, &total_supply, 0, users[0].clone()));
}
#[test]
#[should_panic(expected="HostError: Error(Contract, #117)")] // ContractError::InsufficientAmount
fn mint_negative_shares(){
    ...
    let _ = test.env.as_contract(&vinifica_contract.address, || deposit::test_mint_shares(&test.env, &total_supply, -2000, users[0].clone()));
}
...
fn mint_shares(){
    ...
    let result = test.env.as_contract(&vinifica_contract.address, || deposit::test_mint_shares(&test.env, &total_supply, shares_to_mint, users[1].clone()));
    assert!(result.is_ok());
    assert_eq!(result, Ok(()));
    
    let new_balance = vinifica_contract.balance(&users[1]);
