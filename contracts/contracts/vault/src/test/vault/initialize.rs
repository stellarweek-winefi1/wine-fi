...
use crate::test::{
    create_vinifica_vault, create_hodl_strategy, create_strategy_params_token_0, create_strategy_params_token_1,
    vinifica_vault::{AssetStrategySet, CurrentAssetInvestmentAllocation, RolesDataKey, Strategy, StrategyAllocation}, vinificaVaultTest,
};
fn _create_expected_current_invested_funds(test: &vinificaVaultTest) -> Map<Address, i128> {
...
fn _create_expected_current_idle_funds(test: &vinificaVaultTest) -> Map<Address, i128> {
...
fn get_roles() {
    ...
    let emergency_manager_role = vinifica_contract.get_emergency_manager();

    let vault_assets = vinifica_contract.get_assets();
...
fn with_one_asset_no_strategies(){
    ...
    let _deposit_result = vinifica_contract.deposit(
    ...
    let vault_shares = vinifica_contract.balance(&users[0]);
    let withdraw_amount = vinifica_contract.try_get_asset_amounts_per_shares(&vault_shares).unwrap().unwrap().get(0).unwrap();

    let min_amounts_out = sorobanvec![&test.env, withdraw_amount];
    
    let _withdraw_result = vinifica_contract.withdraw(
        &withdraw_amount,
        &min_amounts_out,
        &users[0].clone(),
    ); 
