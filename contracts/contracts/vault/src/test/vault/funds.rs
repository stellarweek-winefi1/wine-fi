...
use crate::test::{create_vinifica_vault, create_fixed_strategy_params_token_0, vinifica_vault::{AssetStrategySet, RolesDataKey}, vinificaVaultTest};
...
fn report_on_fetch_strategy_invested_funds() {
  ...
  test.token_0_admin_client.mint(&vinifica_contract.address, &amount);

  test.fixed_strategy_client_token_0.deposit(&amount, &vinifica_contract.address);
  ...
  let first_report =  test.env.as_contract(&vinifica_contract.address, || storage::get_report(&test.env, &test.fixed_strategy_client_token_0.address));
  assert_eq!(first_report, expected_first_report);

  let expected_fetch_strategy_invested_funds = Ok(amount);
  let result = test.env.as_contract(&vinifica_contract.address, || funds::fetch_strategy_invested_funds(&test.env, &test.fixed_strategy_client_token_0.address, true));
  ...
  let new_report = test.env.as_contract(&vinifica_contract.address, || storage::get_report(&test.env, &test.fixed_strategy_client_token_0.address));
