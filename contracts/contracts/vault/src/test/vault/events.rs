...
fn check_rebalance_events(){
  ...
  vinifica_contract.rebalance(&test.rebalance_manager, &instructions.clone());
  ...
  let rebalance_events: std::vec::Vec<(Address, Vec<Val>, Val)> = 
    events.iter().filter(
      | event| event.1 == 
        sorobanvec![&test.env, String::from_str(&test.env, "vinificaVault").into_val(&test.env), 
  ...
  let expected_call_params = sorobanvec![
    &test.env,
    (test.strategy_client_token_0.address.clone(), instruction_amount_0, vinifica_contract.address.clone()),
  ];
  ...
  let expected_swap_args:Vec<Val> = sorobanvec![
    ...
    vinifica_contract.address.clone().into_val(&test.env),
  ...
  vinifica_contract.rebalance(&test.rebalance_manager, &swap_exact_out_instructions);
