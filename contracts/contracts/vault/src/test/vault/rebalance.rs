...
use crate::test::vinifica_vault::{ContractError, RolesDataKey, Strategy};
use crate::test::{
    std,
...
fn no_instructions() {
    ...
    let users = vinificaVaultTest::generate_random_users(&test.env, 1);
    test.token_0_admin_client.mock_all_auths().mint(&users[0], &amount);
    let vault_balance = vinifica_contract.balance(&users[0]);
    ...
    let rebalance = vinifica_contract
    .mock_auths(&[MockAuth {
        address: &test.rebalance_manager.clone(),
        invoke: &MockAuthInvoke {
            contract: &vinifica_contract.address.clone(),
...
fn insufficient_balance() {
    ...
    let withdraw_no_funds = vinifica_contract
    .mock_auths(&[MockAuth {
        address: &test.rebalance_manager.clone(),
        invoke: &MockAuthInvoke {
            contract: &vinifica_contract.address.clone(),
    ...
    let invest_no_funds = vinifica_contract
    .mock_auths(&[MockAuth {
        address: &test.rebalance_manager.clone(),
        invoke: &MockAuthInvoke {
            contract: &vinifica_contract.address.clone(),
    ...
    let df_balance: i128 = vinifica_contract.balance(&users[0]);
    assert_eq!(df_balance, amount - 1000);
...
fn swap_exact_out() {
    ...
    let rebalance_result = vinifica_contract
        .mock_auths(&[MockAuth {
            address: &test.rebalance_manager.clone(),
            invoke: &MockAuthInvoke {
                contract: &vinifica_contract.address.clone(),
...
fn swap_wrong_asset_in(){
    ...
    let result = vinifica_contract.try_rebalance(&test.rebalance_manager, &instructions);
...
fn invest_paused_strategy(){
    ...
    let pause_strategy_0 = vinifica_contract.try_pause_strategy(&test.strategy_client_token_0.address, &test.manager);
...
fn invest_more_than_idle_funds(){
    ...
    // Check if invested funds are 0
    let managed_funds = vinifica_contract.fetch_total_managed_funds().get(0).unwrap();
...
fn unwind_paused_strategy(){
    ...
    let unwind_result = vinifica_contract.try_rebalance(&test.rebalance_manager, &instructions);
...
fn should_report(){
    ...
    let initial_report = test.env.as_contract(&vinifica_contract.address, || storage::get_report(&test.env, &test.strategy_client_token_0.address.clone()));
    ...
    let report_after_investment = test.env.as_contract(&vinifica_contract.address, || storage::get_report(&test.env, &test.strategy_client_token_0.address.clone()));
    ...
    vinifica_contract.rebalance(&test.rebalance_manager, &instructions);
    let total_managed_funds_after_unwind = vinifica_contract.fetch_total_managed_funds();
    std::println!("Total managed funds after unwind: {:?}", total_managed_funds_after_unwind);
    // Get report after unwind
    let report_after_unwind = test.env.as_contract(&vinifica_contract.address, || storage::get_report(&test.env, &test.strategy_client_token_0.address.clone()));
    let expected_report = crate::report::Report {
