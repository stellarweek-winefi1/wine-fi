...
use crate::test::{
    create_vinifica_vault, create_hodl_strategy, create_strategy_params_token_0, create_strategy_params_token_1, create_token_contract, get_token_admin_client, vinificaVaultTest, EnvTestUtils
};
...
fn one_asset_no_previous_investment() {
    ...
    let current_idle_funds = test.token_0.balance(&vinifica_contract.address);
    ...
    let current_invested_funds = vinifica_contract.fetch_total_managed_funds().get(0).unwrap().invested_amount;
...
fn several_assets_no_previous_investment() {
    ...
    let vault_df_shares = vinifica_contract.balance(&vinifica_contract.address);
    ...
    // all in idle funds
    let vault_balance0 = test.token_0.balance(&vinifica_contract.address);
    assert_eq!(vault_balance0, amount0);
    // check vault balance of asset 1
    let vault_balance1 = test.token_1.balance(&vinifica_contract.address);
    ...
    // check current idle funds
    let current_idle_funds_token_0 = test.token_0.balance(&vinifica_contract.address);
    assert_eq!(current_idle_funds_token_0, amount0);
    let current_idle_funds_token_1 = test.token_1.balance(&vinifica_contract.address);
    ...
    // check balances after deposit
    let df_balance = vinifica_contract.balance(&users[1]);
...
fn several_assets_wih_previous_investment_success() {
    ...
    // let current_idle_funds = test.vinifica_contract.fetch_current_idle_funds();
    ...
    // let current_invested_funds = test.vinifica_contract.fetch_total_managed_funds();
    ...
    // // user 1 deposits
    // let deposit_result=test.vinifica_contract.deposit(
    //     &sorobanvec![&test.env, amount0_new, amount1_new],
    //     &sorobanvec![&test.env, 0i128, 0i128],
    //     &users[1],
    //     &true,
...
fn one_asset_several_strategies() {
    ...
    */
    ...
    // deposit with no previous investment
    let _deposit0 = vinifica_contract.deposit(
        &sorobanvec![&test.env, deposit_amount],
        &sorobanvec![&test.env, deposit_amount],
        &users[0],
        &true,
    );
    let invested_funds = vinifica_contract.fetch_total_managed_funds().get(0).unwrap().invested_amount;
    let idle_funds = test.token_0.balance(&vinifica_contract.address);
    ...
    let _deposit1 = vinifica_contract.deposit(
...
fn deposit_simple_then_deposit_and_invest() {
    ...
    */
    ...
    let _ = vinifica_contract.deposit(
        &sorobanvec![&test.env, deposit_amount],
        &sorobanvec![&test.env, deposit_amount],
        &users[0],
        &false,
...
fn several_assets_several_strategies() {
    ...
    let users = vinificaVaultTest::generate_random_users(&test.env, 3);
    ...
    let invested_funds_a0 = vinifica_contract.fetch_total_managed_funds().get(0).unwrap().invested_amount;
    let idle_funds_a0 = test.token_0.balance(&vinifica_contract.address);

    assert_eq!(invested_funds_a0, 0i128);
    assert_eq!(idle_funds_a0, deposit_amount_0);

    let invested_funds_a1 = vinifica_contract.fetch_total_managed_funds().get(1).unwrap().invested_amount;
    let idle_funds_a1 = test.token_1.balance(&vinifica_contract.address);

    assert_eq!(invested_funds_a1, 0i128);
    assert_eq!(idle_funds_a1, deposit_amount_1);

    let invested_funds_a2 = vinifica_contract.fetch_total_managed_funds().get(2).unwrap().invested_amount;
    let idle_funds_a2 = token_3.balance(&vinifica_contract.address);
...
fn several_assets_one_strategy_paused() {
    ...
    let invested_funds_a2 = vinifica_contract.fetch_total_managed_funds().get(2).unwrap().invested_amount;
    let idle_funds_a2 = token_2.balance(&vinifica_contract.address);
    ...
    let invested_funds_a2 = vinifica_contract.fetch_total_managed_funds().get(2).unwrap().invested_amount;
    let idle_funds_a2 = test.token_2.balance(&vinifica_contract.address);
    ...
    // Pause the strategy
    vinifica_contract.pause_strategy(&asset_2_strategy.address.clone(), &test.manager.clone());
    
    // Check the strategy is paused
    let strategy = vinifica_contract.get_assets().get(2).unwrap().strategies.get(0).unwrap();
    assert_eq!(strategy.paused, true);

    let amount_0 =  2_0_000_000i128;
    let amount_1 =  2_0_000_000i128;
    let amount_2 =  2_0_000_000i128;
    vinifica_contract.mock_all_auths().deposit(
...
fn several_assets_one_strategy_paused_and_rescue() {
    ...
    let invested_full_funds_a1 = vinifica_contract.fetch_total_managed_funds().get(1).unwrap().invested_amount;
    let idle_funds_a1 = test.token_1.balance(&vinifica_contract.address);
    ...
    // Pause the strategy
    vinifica_contract.pause_strategy(&strategy_1.address.clone(), &test.manager.clone());

    // Check only the strategy_1 is paused
    let strategy_a0 = vinifica_contract.get_assets().get(0).unwrap().strategies.get(0).unwrap();
    assert_eq!(strategy_a0.paused, false);

    let a1_strategy0 = vinifica_contract.get_assets().get(1).unwrap().strategies.get(0).unwrap();
    let a1_strategy1 = vinifica_contract.get_assets().get(1).unwrap().strategies.get(1).unwrap();
    let a1_strategy2 = vinifica_contract.get_assets().get(1).unwrap().strategies.get(2).unwrap();
    ...
    vinifica_contract.rescue(&strategy_1.address, &test.emergency_manager);
