...
use crate::{constants::SCALAR_BPS, test::{
    create_vinifica_vault, create_fixed_strategy, create_strategy_params_token_0, create_strategy_params_token_1, vinifica_vault::{
...
fn negative_amount() {
    ...
    let result = vinifica_contract.try_withdraw(&-100i128, &min_amounts_out, &users[0]);
...
fn with_zero() {
    ...
    vinifica_contract.deposit(&sorobanvec![&test.env, amount_1],&sorobanvec![&test.env, amount_1], &users[0], &false);

    let withdraw_amount = 100i128;
    let min_amounts_out = sorobanvec![&test.env, withdraw_amount];
    let result = vinifica_contract.try_withdraw(&0i128, &min_amounts_out, &users[0]);
    assert_eq!(result, Err(Ok(ContractError::AmountNotAllowed)));
    
    let result = vinifica_contract.withdraw(&withdraw_amount, &min_amounts_out, &users[0]);
...
fn zero_total_supply() {
    ...
    let result = vinifica_contract.try_withdraw(&withdraw_amount, &min_amounts_out, &users[0]);
...
fn not_enough_balance() {
    ...
    assert_eq!(
        vinifica_contract.balance(&users[0]),
        amount_to_deposit - 1000
    );
    assert_eq!(vinifica_contract.balance(&users[1]), amount_to_deposit);
    // check that total supply of vault shares is indeed amount_to_deposit*2
    assert_eq!(vinifica_contract.total_supply(), amount_to_deposit * 2);
...
fn from_idle_one_asset_one_strategy_success() {
    ...
    let total_managed_funds = vinifica_contract.fetch_total_managed_funds();
    ...
    vinifica_contract.withdraw(&amount_to_withdraw, &min_amounts_out, &users[0]);
    ...
    let result = vinifica_contract.try_withdraw(&amount_to_withdraw_more, &min_amounts_out, &users[0]);

    assert_eq!(result, Err(Ok(ContractError::AmountOverTotalSupply)));

    // // withdraw remaining balance
    let result =
        vinifica_contract.withdraw(&(amount_to_deposit - amount_to_withdraw - 1000), &min_amounts_out, &users[0]);
...
fn from_idle_two_assets_success() {
    ...
    assert_eq!(
        test.token_0.balance(&vinifica_contract.address),
        amount_to_deposit_0
    );
    assert_eq!(
        test.token_1.balance(&vinifica_contract.address),
        amount_to_deposit_1
    );
    ...
    let result = vinifica_contract.withdraw(&amount_to_withdraw, &min_amounts_out, &users[0]);
    ...
    assert_eq!(vinifica_contract.get_assets(), expected_asset_vec);
...
fn from_strategy_one_asset_one_strategy_success() {
    ...
    vinifica_contract.rebalance(&test.rebalance_manager, &rebalance_instructions);
    ...
    vinifica_contract.withdraw(&df_balance, &min_amounts_out, &users[0]);
...
fn from_strategies_one_asset_two_strategies_success() {
    ...
    let assets = vinifica_contract.get_assets();
    ...
    let idle_funds_before_withdraw = test.token_0.balance(&vinifica_contract.address);

    let withdraw_amount = amount_0 - 1000;
    let min_amounts_out = sorobanvec![&test.env, withdraw_amount];
        
    vinifica_contract.withdraw(&withdraw_amount, &min_amounts_out, &users[0]);
...
fn from_strategies_two_asset_each_one_strategy_success() {
    ...
    let deposit_result = vinifica_contract.deposit(
        &sorobanvec![&test.env, amount_to_deposit_0, amount_to_deposit_1],
        &sorobanvec![&test.env, amount_to_deposit_0, amount_to_deposit_1],
        &users[0],
        &false,
    ...
    assert_eq!(vinifica_contract.total_supply(), 358024679);
    ...
    // check vault balances
    assert_eq!(test.token_0.balance(&vinifica_contract.address), 0);
    assert_eq!(test.token_1.balance(&vinifica_contract.address), 0);
    ...
    assert_eq!(vinifica_contract.total_supply(), 322671144); //358024679- 35353535
    ...
    let (amounts, shares_minted, asset_allocation) = vinifica_contract.deposit(
        &sorobanvec![
            &test.env,
    ...
    assert_eq!(vinifica_contract.total_supply(), 329115587); //358024679- 35353535 + 6444443
    ...
    // check vault balance
    assert_eq!(test.token_0.balance(&vinifica_contract.address), 2222222);
    assert_eq!(test.token_1.balance(&vinifica_contract.address), 4222222);
    ...
    assert_eq!(vinifica_contract.total_supply(), 328471143); //358024679- 35353535 + 6444443 - 644444
    ...
    assert_eq!(vinifica_contract.balance(&users[0]), 328470143);

    // // Now we will wihdraw the total remineder amount of vault shares of the user
    // // 328471143 - 1000 = 328470143
    let result = vinifica_contract.withdraw(&328470143, &min_amounts_out, &users[0]);
    ...
    assert_eq!(vinifica_contract.balance(&users[0]), 0);
    assert_eq!(vinifica_contract.balance(&vinifica_contract.address), 1000);
...
fn from_strategy_success_no_mock_all_auths() {
    ...
    vinifica_contract.mock_auths(&[MockAuth {
        address: &from.clone(),
    ...
    let idle_funds_0 = test.token_0.balance(&vinifica_contract.address);
    let idle_funds_1 = test.token_1.balance(&vinifica_contract.address);
    ...
    vinifica_contract.mock_auths(&[MockAuth {
        ...
        invoke: &MockAuthInvoke {
            ...
            sub_invokes: &[
                MockAuthInvoke {
                    contract: &vinifica_contract.address,
    ...
    let invested_funds_0 = vinifica_contract.fetch_total_managed_funds().get(0).unwrap().invested_amount;
    let invested_funds_1 = vinifica_contract.fetch_total_managed_funds().get(1).unwrap().invested_amount;
    let idle_funds_0 = test.token_0.balance(&vinifica_contract.address);
    let idle_funds_1 = test.token_1.balance(&vinifica_contract.address);
...
fn unauthorized_withdraw(){
    ...
    let withdraw_response = vinifica_contract.mock_auths(
        &[MockAuth {
...
fn min_amounts_invalid_length(){
    ...
    vinifica_contract.deposit(&deposit_amounts_desired, &deposit_amounts_min, &users[0], &false);
    /*----------------------------------- End of test setup -----------------------------------*/
    ...
    let result = vinifica_contract.try_withdraw(&withdraw_amount, &min_amounts_out_more_than_assets, &users[0]);
    ...
    let result = vinifica_contract.try_withdraw(&withdraw_amount, &min_amounts_out_less_than_assets, &users[0]);
    assert_eq!(result.is_err(), true);
    assert_eq!(result, Err(Ok(ContractError::WrongAmountsLength)));

    /* -------------------------- Try withdraw min_amounts.len() == 0 -------------------------- */
    let min_amounts_out_empty: Vec<i128> = sorobanvec![&test.env];
    let result = vinifica_contract.try_withdraw(&withdraw_amount, &min_amounts_out_empty, &users[0]);
    assert_eq!(result.is_err(), true);
    assert_eq!(result, Err(Ok(ContractError::WrongAmountsLength)));
}
...
fn min_amounts_negative(){
    ...
    let result = vinifica_contract.try_withdraw(&withdraw_amount, &withdraw_min_amounts_out, &users[0]);
    assert_eq!(result.is_err(), true);
    assert_eq!(result, Err(Ok(ContractError::AmountNotAllowed)));
...
fn min_amounts_success(){
    ...
    */
    ...
    let price_per_shares = vinifica_contract.get_asset_amounts_per_shares(&withdraw_amount);
    ...
    test.token_0.transfer(&vinifica_contract.address, &users[1], &(deposit_amount*slippage_bps/SCALAR_BPS as i128));
    test.token_1.transfer(&vinifica_contract.address, &users[1], &(deposit_amount*slippage_bps/SCALAR_BPS as i128));

    let result = vinifica_contract.withdraw(&withdraw_amount, &withdraw_min_amounts_out, &users[0]);
...
fn report_is_ok() {
    ...
    // mint and deposit
    let deposit_amount = 10_0_000_000i128;
    mint_and_deposit(&test, &vinifica_contract, &user, deposit_amount, &test.token_0_admin_client, false);

    // Invest the funds using rebalance instead of direct invest
    invest(&test, &vinifica_contract, deposit_amount);
    ...
    mint_and_deposit(&test, &vinifica_contract, &new_user, new_deposit_amount, &test.token_0_admin_client, true);

    withdraw_all_balance(&test, &vinifica_contract, &new_user);

    let reports = vinifica_contract.report();
...
fn mint_and_deposit(test: &vinificaVaultTest, vinifica_contract: &vinifica_vault::Client, user: &Address, deposit_amount: i128, token_client: &StellarAssetClient, invest: bool) {
...
fn withdraw_all_balance(test: &vinificaVaultTest, vinifica_contract: &vinifica_vault::Client, user: &Address) {
    std::println!("withdraw_all_balance");
    let balance = vinifica_contract.balance(&user);
    vinifica_contract.withdraw(&balance, &sorobanvec![&test.env, 0], &user);
}

fn invest(test: &vinificaVaultTest, vinifica_contract: &vinifica_vault::Client, investment_amount: i128) {
