...
fn budget() {
    ...
    // withdraw
    let min_amounts_out = sorobanvec![&test.env, 2_5_000i128, 2_0_000i128];
    let _ = vinifica_contract.withdraw(&5_0_000i128, &min_amounts_out,&users[0]);
    ...
    test.env.cost_estimate().budget().reset_unlimited();
        ...
        let _ = vinifica_contract.rebalance(&test.rebalance_manager, &invest_instructions);
        let mem = test.env.cost_estimate().budget().memory_bytes_cost();
        let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
        std::println!(
    ...
    let _ = vinifica_contract.rebalance(&test.rebalance_manager, &withdraw_instructions);
    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
    std::println!(
    ...
    let _ = vinifica_contract.rebalance(&test.rebalance_manager, &several_instructions_one_strategy);
    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
    std::println!(
    ...
    let _ = vinifica_contract.rebalance(&test.rebalance_manager, &several_instructions_two_strategy);
    let mem = test.env.cost_estimate().budget().memory_bytes_cost();
    let cpu = test.env.cost_estimate().budget().cpu_instruction_cost();
    std::println!(
